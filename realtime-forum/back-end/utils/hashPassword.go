...
func NewHashPassword(memory, iterations uint32, parallelism uint8, saltLength, keyLength uint32) *HashParams {
	return &HashParams{
		Memory:      memory,
...
func (h *HashParams) GenerateHashPassword(password string) (string, error) {
...
func ComparePasswordHash(password, encodedHash string) (match bool, err error) {
	...
	if subtle.ConstantTimeCompare(hash, otherHash) == equal {
		return true, nil
	}
